...
const App: React.FC = () => {
  ...
  // Ganti Favicon menjadi logoui.svg
  useEffect(() => {
    const link = document.querySelector("link[rel~='icon']") as HTMLLinkElement;
    if (link) {
      link.href = '/logoui.svg';
    } else {
      const newLink = document.createElement('link');
      newLink.rel = 'icon';
      newLink.href = '/logoui.svg';
      document.head.appendChild(newLink);
    }
  }, []);

  // Computed Stats Peraturan
  ...
  }, [selectedPeraturan, showAllPeraturan, showAboutUs]);
  ...
  const openAddEventForm = () => {
    const initialDate = selectedCalendarDate || today;
    setEventFormData({
  ...
  const renderCalendarDays = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const isToday = isSameDate(today, dateObj);
  ...
  // Image Swiper Component (Mendukung Horizontal Swipe & Vertical Scroll)
  const ImageSwiper: React.FC = () => {
    const images = ['/pi.svg', '/kominfo.svg', '/kokum.svg'];
    const cardWidth = 300; 
  ...
  return (
    <>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .navbar {
          ...
          background: ${scrolled || showAllPeraturan || selectedPeraturan || showAboutUs ? 'rgba(0,0,0,0.85)' : 'transparent'};
          backdrop-filter: blur(10px);
          box-shadow: ${scrolled || showAllPeraturan || selectedPeraturan || showAboutUs ? '0 4px 30px rgba(0,0,0,0.2)' : 'none'};
          height: ${scrolled || showAllPeraturan || selectedPeraturan || showAboutUs ? '56px' : '64px'};
          border-bottom: 1px solid ${scrolled || showAllPeraturan || selectedPeraturan || showAboutUs ? 'var(--hairline-strong)' : 'transparent'};
        }

        .nav-logo { display: flex; align-items: center; gap: 12px; cursor: pointer; margin-right: auto; }
        .nav-logo img { height: 40px; transition: height 0.3s ease; filter: drop-shadow(0 0 4px rgba(255, 255, 255, 0.3)); }
        ...
        .menu-toggle span { width: 24px; height: 2px; background: var(--on-dark); transition: all 0.3s ease; border-radius: 2px; }
        ...
        .btn-logout { background: transparent; color: var(--status-error); border: 1px solid var(--status-error); padding: 4px 12px; border-radius: var(--radius-sm); font-weight: 700; font-size: 12px; cursor: pointer; text-transform: uppercase; font-family: var(--font-heading); }

        /* HERO & STATS */
        .hero {
          position: relative; height: 100vh; min-height: 600px; display: flex; align-items: center;
          justify-content: flex-start; text-align: left;
          background: url('/gedungfia.svg') center center / cover no-repeat;
        ...
        .hero-badge {
          display: inline-block; background: var(--primary-gold); color: var(--ink); padding: 6px 16px;
          border-radius: var(--radius-sm); font-size: 14px; font-weight: 700; margin-bottom: 24px; text-transform: uppercase; font-family: var(--font-heading);
        }
        ...
        .btn-primary {
          display: inline-flex; background: var(--primary-gold); color: var(--ink); padding: 11px 24px;
          text-decoration: none; font-weight: 700; border-radius: var(--radius-md); border: none; cursor: pointer;
          font-family: var(--font-heading); font-size: 16px; transition: all 0.2s; text-transform: uppercase; height: 44px; align-items: center; justify-content: center;
          box-shadow: 0 4px 6px rgba(0,0,0,0.1);
        }
        .btn-primary:hover { background: var(--primary-gold-dark); transform: translateY(-1px); box-shadow: 0 6px 8px rgba(0,0,0,0.15); }

        .stats-section {
          background: var(--canvas); padding: 32px; position: relative; z-index: 3;
        ...
        .card {
          background: var(--canvas); overflow: hidden; border: 1px solid var(--hairline);
          border-radius: var(--radius-lg); transition: all 0.3s ease; display: flex; flex-direction: column; position: relative;
        ...
        .card-header {
          background: var(--ink); padding: 12px 24px; color: var(--on-dark); font-family: var(--font-heading); font-weight: 700;
          display: flex; justify-content: space-between; align-items: center; font-size: 14px; text-transform: uppercase;
        }
        ...
        .btn-outline {
          display: inline-flex; border: 2px solid var(--primary-gold); color: var(--primary-gold); padding: 8px 13px;
          text-decoration: none; font-weight: 700; font-size: 14.4px; border-radius: var(--radius-md); transition: all 0.2s;
          cursor: pointer; background: transparent; font-family: var(--font-heading); text-transform: uppercase; align-self: flex-start; margin-top: auto;
        }
        .btn-outline:hover { background: var(--primary-gold); color: var(--ink); }

        .btn-more-container { text-align: center; margin-top: 48px; }
        .btn-back {
          display: inline-flex; align-items: center; gap: 8px; margin-bottom: 32px; color: var(--primary-gold);
          font-weight: 700; font-size: 16px; cursor: pointer; background: none; border: none; font-family: var(--font-heading); text-transform: uppercase;
        }

        /* IMAGE SWIPER (ABOUT US) */
        .image-card {
          position: absolute;
          place-self: center;
          border: 1px solid var(--hairline);
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave}>
              <div className="form-group">
                ...
                <input type="text" className="form-input" required value={formData.judul} onChange={e => setFormData({...formData, judul: e.target.value})} />
              </div>
              <div className="form-group">
                <label className="form-label">Jenis Dokumen</label>
                <input type="text" className="form-input" required value={formData.jenisDokumen} onChange={e => setFormData({...formData, jenisDokumen: e.target.value})} />
              </div>
            ...
            <form onSubmit={handleEventSave}>
              ...
              <div className="form-group">
                ...
                <textarea 
                  ...
                  required 
                  rows={4}
                  placeholder="Deskripsi singkat / berita event"
                  style={{resize: 'vertical', fontFamily: 'var(--font-body)'}}
                  value={eventFormData.eventNews} 
                  onChange={e => setEventFormData({...eventFormData, eventNews: e.target.value})} 
                />
              </div>
              <div className="form-actions">
                <button type="submit" className="btn-primary" style={{flex:1}}>Simpan</button>
      ...
      {/* NAVBAR */}
      <nav className={`navbar ${scrolled || showAllPeraturan || selectedPeraturan || showAboutUs ? 'scrolled' : ''} ${(showAllPeraturan || selectedPeraturan || showAboutUs) ? 'page-peraturan' : ''}`}>
        ...
        <ul className={`nav-links ${menuOpen ? 'active' : ''}`}>
          ...
          <li>
            <a className="mobile-admin-link" onClick={() => { setShowLogin(true); setMenuOpen(false); }}>Admin</a>
          </li>
        </ul>
      ...
      {/* KONDISI HALAMAN */}
      {!showAllPeraturan && !selectedPeraturan && !showAboutUs ? (
        /* HALAMAN BERANDA */
        <>
          ...
          {/* SECTION ISS (INTERNAL SCHEDULING SYSTEM) */}
            ...
            <div className="iss-container">
              <div className="calendar-header">
                <div className="calendar-title">
                  {monthsID[currentDate.getMonth()]} {currentDate.getFullYear()}
                </div>
              ...
              <div className="calendar-events">
                {selectedCalendarDate ? (
                  <>
                    ...
                    {selectedDateEvents.length > 0 ? (
                      selectedDateEvents.map(ev => (
                        <div key={ev.id} className="event-item">
                          ...
                          <p>{ev.description}</p>
                          {isAdmin && (
                            <button className="btn-delete" style={{ marginTop: '12px' }} onClick={() => handleDeleteEvent(ev.id)}>
                              Hapus Event
                            </button>
                          )}
                        </div>
                    ...
                    {isAdmin && (
                      <button className="btn-primary add-event-btn" onClick={openAddEventForm}>
                        + Tambah Event Baru
                      </button>
                    )}
                  </>
          ...
          <section className="section">
            ...
            <div className="detail-layout">
              ...
              <div className="info-box">
                ...
                <div className="info-row">
                  ...
                  <div className="info-value">
                    <span className={`status-badge ${selectedPeraturan.status === 'Berlaku' ? 'status-berlaku' : selectedPeraturan.status === 'Dicabut' ? 'status-dicabut' : 'status-tidak-berlaku'}`}>
              ...
              <div className="action-buttons">
                ...
                <button className="action-btn outline">
                  {/* Ikon Jam dengan Panah Kembali (History) */}
          ...
          <div className="footer-about">
            ...
            <div className="social-links">
              ...
              <a href="https://lin.ee/pteZwX4" target="_blank" rel="noopener noreferrer" title="Line">
                {/* Logo LINE Menggunakan File Eksternal */}
                <img src="/line.png" alt="Line" width="20" height="20" style={{ filter: 'brightness(0) invert(1)' }} />
              </a>
            </div>
          </div>
